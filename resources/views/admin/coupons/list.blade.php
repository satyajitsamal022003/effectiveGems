                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="onoffswitch">
                                                <input type="checkbox" name="onoffswitch{{ $coupon->id }}" class="onoffswitch-checkbox" id="couponStatus{{ $coupon->id }}" tabindex="0" {{ $coupon->status ? 'checked' : '' }} onchange="toggleCouponStatus({{ $coupon->id }})">
                                                <label class="onoffswitch-label" for="couponStatus{{ $coupon->id }}"></label>
                                            </div>
                                        </td>
                                        <td class="center action">
                                            <a href="{{ route('coupons.edit', $coupon->id) }}" class="btn btn-sm bg-success mr-2">
                                                <i class="fa fa-pencil"></i>
                                            </a>
                                            <a class="btn btn-sm bg-danger" href="{{ route('coupons.destroy', $coupon->id) }}" onclick="return confirm('Are you sure to delete this coupon?');" title="Delete">
